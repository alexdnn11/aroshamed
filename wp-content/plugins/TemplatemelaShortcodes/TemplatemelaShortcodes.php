...
function shortcode_testimonials($atts, $content = null, $code) {
	...
	foreach($testimonial_array as $post) : setup_postdata($post);	
		...
		$output .= '<div class="item'.$class.'"><div class="product-block">';
			...
			elseif($style == 2)
			{
				$output .= '<div class="single-testimonial2">';			
					$output .= '<div class="testimonial-content2">';
						$output .= '<div class="testimonial-top2" style="'.$background_color.'"><blockquote><q>'.substr($contents, 0, 150).'</q></blockquote></div>';
						$output .= '<div class="testimonial-bottom2"></div>';
					$output .= '</div>';
					$output .= '<div class="testmonial-other2">';
						$output .= '<div class="testmonial-text2">';			
							$output .= '<div class="testimonial-title2">'.get_the_title().'</div>';
							if(!empty($testimonial_position)):
								if(!empty($testimonial_link)):
									$output .= '<div class="testimonial-position2"><a target="_Blank" title="'.$testimonial_position.'" href="'.$testimonial_link.'" >'.$testimonial_position.'</a></div>';	
								else:
									$output .= '<div class="testimonial-position2">'.$testimonial_position.'</div>';
								endif;
							endif;
						$output .= '</div>';	
					$output .= '</div>';	 
	...
	if(!empty($category)):	
		...
		$term = get_term( $term_id, 'faq_categories' );		
		 ...
		 while (have_posts()) : the_post(); 
			...
			$output .= '<a class="tog" href="#">'. get_the_title() .'</a>';
...
function shortcode_two_fourth($atts, $content = null){
	...
	$output = '';
	$output .= '<div class="two_fourth">';	
	$output .= '<div class="two_fourth_inner content_inner '.$align.'" style="margin:'.$margin.';width:'.$content_width.';">';
	$output .=	do_shortcode($content);
	$output .= '</div>';
	$output .= '</div>';
	return $output;
}
add_shortcode('two_fourth', 'shortcode_two_fourth');
		...
		if( empty ( $src ) || $src == 'image_not_specified' ):
			...
			$output .= '<div class="container-inner">';
				$output .= '<div class="post-image">';
					...
					$output .= '<img src="'.$src.'" title="'.get_the_title().'" alt="'.get_the_title().'" /></a>';
					$output .= '<div class="block_hover">';
					$output .= '<div class="links">';				
					$output .= '<a href="'.$image.'" class="icon mustang-gallery"><i class="fa fa-search"></i></a>';				
					$output .= '<a href="'.get_permalink().'" class="icon"><i class="fa fa-link"></i></a>';
				...
				$output .= '<div class="post-date"><i class="blog-icon fa fa-calendar"></i><div class="day">'.$post_day.'</div><div class="month">'.$post_month.'</div> <div class="year">'.$post_year. '</div></div>';
				$output .= '</div>';
				$output .= '<div class="post-content-inner">';
				$shorttitle = mb_substr(get_the_title('','',FALSE),0,50);
...
//			if($comments->total_comments > 0):
//          		$output .='<div class="comments-link">'.$comments->total_comments.' comment </div>';
//       		else:
//          		$output .='<div class="comments-link">Leave a comment </div>';
//         	endif;
			$output .= '</div></div></div>';
		$i++;
	endwhile;
