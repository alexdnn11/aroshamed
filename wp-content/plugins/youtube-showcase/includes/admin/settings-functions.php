		...
		foreach($map_ents as $kent => $myent){
			...
			if(!empty($file_attrs[$kent])){
				foreach($file_attrs[$kent] as $fattr => $fval){
					...
					echo "<p class='description'>" . sprintf(__('Sets the maximum uploadable file size in kilobytes. Your server allows up too %s KB. Leave it blank to use server limit.','emd-plugins'),$server_size) . "</p></td></tr>";
					...
					echo "<p class='description'>" . __('Sets the file extensions allowed to upload. Seperate each extension by a comma.','emd-plugins') . "</p></td></tr>";
				}
			}	
			//comments
			...
			if(!empty($comment_list) && !empty($comment_list[$kent])){
				...
				echo sprintf(__('Set the depth of threaded (nested) %s','emd-plugins'),strtolower($comment_list[$kent]['label']));
				echo '</label></th><td>';
				...
				echo sprintf(__('%s per page','emd-plugins'),$comment_list[$kent]['label']);
				echo '</label></th><td>';
				$per_page_val = '10';
...
if (!function_exists('emd_ent_map_sanitize')) {
	function emd_ent_map_sanitize($input){
		...
		$map_keys = apply_filters('emd_entity_map_keys',Array('hide_archive_page_nav','hide_prev_next','hide_rels','hide_rel_attrs','attrs','single_temp','archive_temp','single_container','archive_container','make_visitor_visible','rewrite','cust_fields','width','height','zoom','map_type','marker','load_info','thread_depth','max_files','max_file_size','file_exts','comments_per_page','comments_labels','comments_attach','comments_attach_visitor','comments_max_file_size','comments_max_files','comments_file_exts'),$input['app']);
...
if (!function_exists('emd_tax_settings_sanitize')) {
	function emd_tax_settings_sanitize($input){
		...
		$keys = Array('rewrite','temp','container','hide','hide_page_nav');
		foreach($input as $ikey => $vkey){
			if($ikey != 'app'){
