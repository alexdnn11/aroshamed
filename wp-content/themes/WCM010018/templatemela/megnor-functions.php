...
function tm_set_default_options()
{
	...
	add_option("tmoption_logo_image_alt",'Fringe'); // set logo image alt
	...
	add_option("tmoption_button_color","7aa8cc"); // button color
	...
	add_option("tmoption_link_color","606060"); // link color
	add_option("tmoption_hoverlink_color","7aa8cc"); // link hovre color
	/*  Footer Settings  */	
	add_option("tmoption_footerbkg_color","FFFFFF"); // footer background color
	add_option("tmoption_footerlink_color","606060"); // footer link text color
	add_option("tmoption_footerhoverlink_color","7AA8CC"); // footer link hover text color
	...
	add_option("tmoption_crosssells_items","9"); 	
	/* Slider settings */
...
function templatemela_excerpt($limit) 
{
      ...
      if (count($excerpt)>=$limit) {
        array_pop($excerpt);
        $excerpt = implode(" ",$excerpt).'&nbsp;.<div class="read-more"><a class="read-more-link" href="'.get_permalink().'">ПОДРОБНЕЕ...</a></div>';
      } else {
...
function templatemela_load_fonts() {
	...
	wp_enqueue_style( 'roboto-user', "$protocol://fonts.googleapis.com/css?family=Roboto Slab:300,400,500,600,700" );
	wp_enqueue_style( 'glegoo-user', "$protocol://fonts.googleapis.com/css?family=Glegoo:300,400,500,600,700" );
}
endif;
...
function templatemela_get_topbar_banner() {	
	...
	$output .= '<ul class="top-banner-container">';
		...
		$output .= '<li class="content1 content"><div class="content-inner"><div class="fa fa-phone"></div><div class="cms"><div class="cms-title">'.$tmoption_custom_text1.'</div> <div class="cms-desc">'.$tmoption_custom_text1_desc.'</div></div></div></li>';
		endif;
		if (!empty($tmoption_custom_text2)):
		$output .= '<li class="content2 content"><div class="content-inner"><div class="fa fa-mobile"></div><div class="cms"><div class="cms-title">'.$tmoption_custom_text2.'</div> <div class="cms-desc">'.$tmoption_custom_text2_desc.'</div></div></div></li>';
		endif;
		if (!empty($tmoption_custom_text3)):
		$output .= '<li class="content3 content"><div class="content-inner"><div class="fa fa-envelope"></div><div class="cms"><div class="cms-title">'.$tmoption_custom_text3.'</div> <div class="cms-desc">'.$tmoption_custom_text3_desc.'</div></div></div></li>';
		endif;	
	$output .= '</ul>';	
	$output .= '</div>';
	echo $output;
...
function tm_post_entry_date( ) {
	...
	$date = '<div class="entry-date"><div class="day">'.$day.' '.$month.'</div></div>';
	echo $date;
	return $date;
}
endif;
...
function tm_posts_short_description() {
	...
	if(empty($tm_posts_short_description))
		$tm_posts_short_description = templatemela_excerpt(55);
...
add_image_size( 'blog-posts-list', 400, 300, true );
...
/* Upsell Product settings */
	...
	function templatemela_output_crosssell() {
		$no1 = get_option("tmoption_crosssells_items");	
...
function tm_googleMap($latlong, $text, $pin, $height, $id, $class) {
        ...
        <script type="text/javascript" src="http://maps.google.com/maps/api/js?key=AIzaSyAGNFESrssCCVGfPwsGfOOymTAAtzJjVig&sensor=false"></script>
        <script type="text/javascript">       
